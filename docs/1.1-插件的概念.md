...
- **Gradle Wrapper 又是什么？Gradle 不需要下载安装吗？** 由于 Gradle 本身是多变的，包括其版本和调用方式，所以出现了 Wrapper（包装器）的概念，也就是对 Gradle 进行包装，包装以后的结果就是一个统一调用方式的程序，以便于使用。Gradle Wrapper 在我们的项目中起到的作用就是调用 Gradle 本体进行一系列的操作（如构建）。
